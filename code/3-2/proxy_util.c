 ...
 * @date Thu Jun  7 13:04:51 KST 2018
 */
...
/**
 * A thread function for writing file
 * @param log_struct A structure containing FILE pointer and message pointer
 */
void *write_file(void *log_struct){
    // cast to the appropriate type
    Log_struct *ls = (Log_struct *)log_struct;

    // write and flush
    fwrite(ls->buf, 1, strlen(ls->buf), ls->fp);
    fflush(ls->fp);
}
...
int write_log(const char *path, const char *header, const char *body, bool time_flag, bool pid_flag){
    ...
    // create thread with write_file function and pass the log struct
    if(pthread_create(&tid, NULL, write_file, (void *)&ls) != 0){
    ...
    // wait the thread made before and join it
